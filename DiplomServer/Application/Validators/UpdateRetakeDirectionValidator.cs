...
{
    public class UpdateRetakeDirectionValidator : AbstractValidator<UpdateRetakeDirectionRequestDto>
    {
        public UpdateRetakeDirectionValidator()
        {
            RuleFor(x => x.Number)
                .NotEmpty().WithMessage("Номер направления обязателен.")
                .MaximumLength(100).WithMessage("Номер направления не должен превышать 100 символов.");
